...
//			counselDto.setScheduleType("100562"); // 상담 -- 나중에 검사도 체크해야됨 우선은 상담만
	 ...
	 * @return
	 */
	public Paging<CounselReservDto> getCounselReservList(CounselReservDto dto) {
		
		Paging<CounselReservDto> paging = new Paging<CounselReservDto>();
		paging.setPaging(dto);
		paging.setList(counselMapper.getCounselReservList(dto));
		dto.setPageCommonFlag(true);
		paging.setTotalCount(counselMapper.getCounselReservList(dto).get(0).getPageCommonCount());
	...
	public int updateCounselReservStatus(CounselReservDto dto) {
		...
		int result = counselMapper.updateCounselReservStatus(dto);
		
		if(result > 0) { // TODO mail or mobile sms action 
	...
	public int countByCounselToChildren(CounselDto counselDto) {
		return counselMapper.countByCounselToChildren(counselDto);
	}
}
