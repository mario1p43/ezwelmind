...
@Controller
@RequestMapping("/madm/clientDivision")
public class ClientDivisionController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/divisionMain")
	public String menuManager(String clientCd, Model model) {
		...
		return "madm/client/division/divisionMain";
	}
	...
	public ModelAndView getMenuList(String clientCd, Model model) {
		...
		if (divisionList.size() != 0)
			mv.addObject("josonData", JsonUtils.convertorTreeMap(divisionList, "100002", "commCd", "highCommCd", "commNm", "orderNo"));
		else
	...
	public String addMenuTree(@ModelAttribute ClientDivisionDto clientDivisionDto) throws Exception {
		clientDivisionService.addDivisionTree(clientDivisionDto);
		return "redirect:/madm/clientDivision/divisionMain?clientCd=" + clientDivisionDto.getClientCd();
	}
