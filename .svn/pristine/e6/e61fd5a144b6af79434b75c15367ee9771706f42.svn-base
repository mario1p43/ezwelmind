	...
	j$('.modify').click(function() {
		...
		j$.alert("변경 하시겠습니까?", function() {
			j$('input:checkbox[name="goodsCdArr"]:unchecked').each(function(){
			 ...
			 j$("#addGoodsForm").attr("action","/partner/service/management/modify");
			 j$("#addGoodsForm").submit();
		}, function() {
		}); // end alert
	});
...
function detail(Cd,Type){
	j$("#addGoodsForm").attr("action","/partner/service/management/modifyGoods?goodsCd="+Cd+"&goodsType="+Type);
	j$("#addGoodsForm").submit();
}
...
<form id="addGoodsForm" name="addGoodsForm" action="/partner/service/management/addgoods" method="POST">
