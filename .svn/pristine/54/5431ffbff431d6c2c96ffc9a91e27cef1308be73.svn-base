...
j$(document).ready(function(){
	...
	j$('#updateBtn').click(function() {
		j$("#addNotice").attr("action","/madm/sangdam4uNotice/update");
		oEditors[0].exec("UPDATE_CONTENTS_FIELD", []);   // 에디터의 내용이 textarea에 적용된다.
	...
	j$('#deleteBtn').click(function() {
		j$("#addNotice").attr("action","/madm/sangdam4uNotice/delete");
		j$.alert('삭제하시겠습니까?',function(){
			location.href = "/madm/sangdam4uNotice/delete?dataSeq=" + j$('#dataSeq').val();
		 },function(){
		 	return false;
		 }
 	);
	...
	j$("#cancleBtn").click(function(){
		location.href = "/madm/sangdam4uNotice/list?bbsCd=notice&serviceType=SANGDAM4U";
		return false;
	});
...
<form id="addNotice" name="addNotice"  action="/madm/sangdam4uNotice/add" method="POST" enctype="multipart/form-data">
