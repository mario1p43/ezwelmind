...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				aPhotoInfo = {
				    ...
				    originalName : decodeURIComponent(htData.originalName) || "",
				    sOriginalImageURL : htData.sFileURL,
					bNewLine : htData.bNewLine || false 
				};
				
				if(htData.fileType === "excel"){
					sContents += this._getExcelTag(aPhotoInfo);
				}else{
	 ...
	 * @use 엑셀파일 tag 생성
	 */
	_getExcelTag : function(htPhotoInfo){
		var sTag = '<a href="{=sOriginalImageURL}"><span style="background-color:#00FF00; color:red">{=originalName} (엑셀파일저장)</span></a>';
		if(htPhotoInfo.bNewLine){
			sTag += '<br style="clear:both;">';
		}
		sTag = jindo.$Template(sTag).process(htPhotoInfo);
