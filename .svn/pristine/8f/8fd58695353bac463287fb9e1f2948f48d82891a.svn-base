...
j$(document).ready(function(){
	...
	j$('#updateBtn').click(function() {
		j$("#addNotice").attr("action","/madm/partnerNotice/update");
		oEditors[0].exec("UPDATE_CONTENTS_FIELD", []);   // 에디터의 내용이 textarea에 적용된다.
	...
	j$('#deleteBtn').click(function() {
		j$("#addNotice").attr("action","/madm/partnerNotice/delete");
		j$.alert('삭제하시겠습니까?',function(){
			location.href = "/madm/partnerNotice/delete?dataSeq=" + j$('#dataSeq').val();
		 },function(){
		 	return false;
		 }
 	);
	...
	j$("#cancleBtn").click(function(){
		location.href = "/madm/partnerNotice/list?bbsCd=notice&serviceType=SANGDAM4U";
		return false;
	});
...
<form id="addNotice" name="addNotice"  action="/madm/partnerNotice/add" method="POST" enctype="multipart/form-data">
