...
j$(document).ready(function(){
	...
	j$('#updateBtn').click(function() {
		j$("#addGallery").attr("action","/madm/bbsGallery/update");
		oEditors[0].exec("UPDATE_CONTENTS_FIELD", []);   // 에디터의 내용이 textarea에 적용된다.
	...
	j$('#deleteBtn').click(function() {
		j$("#addGallery").attr("action","/madm/bbsGallery/delete");
		j$.alert('삭제하시겠습니까?',function(){
			location.href = "/madm/bbsGallery/delete?dataSeq=" + j$('#dataSeq').val();
		 },function(){
		 	return false;
		 }
 	);
	...
	j$("#cancleBtn").click(function(){
		...
		location.href = "/madm/bbsGallery/list?bbsCd="+bbsCd+"&serviceType="+serviceType;
		return false;
	});
...
<form id="addGallery" name="addGallery"  action="/madm/bbsGallery/add" method="POST" enctype="multipart/form-data">
