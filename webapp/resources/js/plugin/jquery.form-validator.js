...
(function ($) {
    ...
    _setInlineErrorMessage = function ($input, mess, conf, $messageContainer) {
      var custom = document.getElementById($input.attr('name') + '_err_msg'),
          setErrorMessage = function($elem) {
            ...
            //$elem.html(mess);
            if (conf.validateOnAlert && alertCnt !== -1) {
	            if (mess != "") {
	            	alertCnt++;
	            	if (alertCnt < 2) {
	            		alert(mess);            		
            ...
            if (conf.validateOnSpan || conf.validateOnBlur) {
            	$elem.html(mess);
            }
          };
  ...
  $.validate = function (conf) {
    ...
    var defaultConf = $.extend($.formUtils.defaultConfig(), {
      ...
      validateOnAlert: false,
      validateOnSpan: false,
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function (val, $el, conf) {
    	var check = /^[가-힣]+$/
        var result = check.test(val);
        if (result) {
        	return true; //한글일 경우
        } else {        	
        	this.errorMessage = "한글을 입력해주세요.";
  ...
  $.formUtils.addValidator({
    name: 'mobile',
    validatorFunction: function (val, $el, conf) {
    	var check = /[01](0|1|6|7|8|9)[-](\d{4}|\d{3})[-]\d{4}$/g;
        var result = check.test(val);
        if (result) {
        	return true; //한글일 경우
        } else {        	
        	this.errorMessage = "휴대폰 번호를 입력해주세요.";
        	return false; //한글이 아닐경우
        }
    },
    errorMessage: '',
