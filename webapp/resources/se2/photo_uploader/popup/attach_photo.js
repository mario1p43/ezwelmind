    ...
    function html5Upload() {	
    	var tempFile,
    		sUploadURL;
    	sUploadURL= '/common/file/upload?isHtml5=true&isImageServer=true'; 	//upload URL
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	       ...
 	       // sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sUrl : '/common/file/upload?isImageServer=true',
 		    sCallback: '/resources/se2/photo_uploader/popup/callback.html',
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif;*.xlsx;*.xls",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP, xlsx, xls 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		console.log("oCustomEvent: " + oCustomEvent);
