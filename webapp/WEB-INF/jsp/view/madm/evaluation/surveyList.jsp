...
<html>
<head>
<title>만족도조사(약식)</title>
<script type="text/javascript">
j$(document).ready(function(){
	clientReload('sel');
	j$(".downBtn").click(function(){
		j$(".downBtn").hide();
		j$("#frm").attr("action","/madm/evaluation/excelDownload");
		j$("#frm").submit();
	});
	
	$("#searchBtn").click(function(){
		j$("#frm").attr("action","/madm/evaluation/surveyList");
		j$("#frm").submit();
		return false;
	});
	...
	$('.item02Ans').click(function() {
			var gSize = "";

			$(".item02Ans:checked").each(function() {
	...
	$('.checkboxAll').click(function() {
		if($('.checkboxAll').is(':checked') == true){
			$('.selectSurvey').prop("checked", true);
		}else{
			$('.selectSurvey').prop("checked", false);
		}
...
function updateShowYn(selectYn){
	if( j$("input[class=selectSurvey]:checked").length == 0 ) {
		 j$.alert("선택된 값이 없습니다.");
		 return;
	}
	
	var chk = document.getElementsByName("selectSurvey");
	var targetSeqAll = document.getElementsByName("targetSeq");
	console.log(targetSeqAll);
	var evalSeqAll = document.getElementsByName("evalSeq");
	var userKeyAll = document.getElementsByName("userKey");
	var showYn  = "N";
	if(selectYn=='0'){
		showYn = "N";
	}else{
	...
	for(var i=0; i<chk.length;i++){
		if(chk[i].checked == true){  //체크가 되어있는 값 구분
			...
			var targetSeq = targetSeqAll[i].value;
			var evalSeq = evalSeqAll[i].value;		
			var params = {};
...
<body>
<form:form id="frm" modelAttribute="evaluationDto" action="/madm/evaluation/surveyList" method="GET">
...
<tr>
	<td>
		...
		<table cellpadding="5" cellspacing="0" border="0" width="100%" align="center" style="border-bottom: 1px solid silver;">
			<tr>
		    	<td align="left" class="subtitle">만족도조사(약식)</td>
			</tr>
		</table>
		<!-- 버튼 영역 종료 -->
	</td>
...
<tr>
	<td>
		...
		<table cellpadding="5" cellspacing="0" border="1" width="65%" style="border-collapse:collapse;" bordercolor="#DDDDDD">
			...
			<tr>
				<td  width="" bgcolor="#F5F5F5" align="center"><strong>승인상태</strong></td>
				<td>
					<select name="showYn" >
                     	<option value="">---- 선택 ----</option>
				...
				<td  width="" bgcolor="#F5F5F5" align="center"><strong>평점기준</strong></td>
				<td>
					<input type="hidden" name="item02Ans">
					<input type="checkbox" class="item01Ans" value="0" />0
					<input type="checkbox" class="item02Ans" value="1" />1
                    <input type="checkbox" class="item02Ans" value="2" />2
                    <input type="checkbox" class="item02Ans" value="3" />3
                    <input type="checkbox" class="item02Ans" value="4" />4
                    <input type="checkbox" class="item02Ans" value="5" />5
				</td>
			</tr>
		</table>
		<!-- 검색 영역 종료 -->
					...
					<table cellpadding="5" cellspacing="0" border="0" width="100%" style="border-collapse:collapse; margin: 15px auto 5px auto;">
						<tr>
							...
							<td align="left" id="btn02" class="hideBtn" width="auto;">
								...
								<a style="margin-left: 20px" href="javascript:updateShowYn(1);">
									<span> 공개승인 </span>
								</a>
								<a style="margin-left: 20px" href="javascript:updateShowYn(0);">
									<span> 승인취소 </span>
								</a>
			...
			<tr>
				<td>
					...
					<table class="" cellpadding="5" align="" id="chRow" cellspacing="0" border="1" width="100%" style="text-align:center; border-collapse:collapse;">
						<thead>
			                <tr>
								<th bgcolor="#F5F5F5" width="30px"><strong>선택</strong><input type="checkbox" class="checkboxAll" /></th>
			                  <th width="40px">순번</th>
			                  ...
			                  <th width="60px">승인상태</th>
			                  <th width="130px">공개승인 일자<br/>(승인자)</th>
			                </tr>
			            </thead>
						...
						<c:forEach var="list" items="${paging.list}" varStatus="status">
							<tr align="" height="50px">
								<td>
								<input type="checkbox" name="selectSurvey" class="selectSurvey" />
								<input type="hidden" name="targetSeq" value="${list.targetSeq }" />
								...
								<td>${list.regDt }</td>
								<td>
								<c:if test="${list.showYn eq 'Y'}">
								승인
								</c:if>
								<c:if test="${list.showYn eq 'N'}">
								미승인
								</c:if>
								
								</td>
								<td>${list.showDt }/<br/>(${list.showId })</td>
							</tr>
						</c:forEach>
					</tbody>
					</table>
					</div>
					<div>평균 : ${itemAvg.itemAvg } / 최고점 : ${itemAvg.itemMax } / 최하점 : ${itemAvg.itemMin } / (1번 응답:N 미포함)</div>
					<br>
