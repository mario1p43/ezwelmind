...
j$(document).ready(function(){
	...
	j$('.searchbtn').click(function() {
		 j$("#subGoodsForm").attr("action","/madm/service/management/getSubGoodsList");
	...
	$('.modify').click(function() {
		...
		if(confirm("변경 하시겠습니까?")){
			j$('input:checkbox[name="orderNumArr"]:unchecked').each(function(){
			 ...
			 j$("#subGoodsForm").attr("action","/madm/service/management/modifySubOrderStatus");
			 j$("#subGoodsForm").submit();
		}
	});
	...
	j$(".downBtn").click(function(){
		j$(".downBtn").hide();
		j$("#subGoodsForm").attr("action","/madm/service/management/getExcelSubGoodsList");
		j$("#subGoodsForm").submit();
	});
	
});

function goOrderCancel(orderNum) {
	var returnValue = confirm("주문을 취소하시겠습니까?");
	if (returnValue) {
		$.ajax({
		    type: "GET",
		    data: {"orderNum":orderNum,"regId":"${userId}"},
		    dataType: 'jsonp',
			url: "${sangdam4uUrl}/cancel/adminCancelOrder"
		});
		setTimeout(function (){
			location.reload();
		},3000);
...
function getcspPopup() {
	window.open("/madm/service/management/cspPopup?Oform=csp", "cspPopup", "width=600, height=400,  scrollbars=1");
}
...
function getClientPopup() {
	window.open("/madm/service/management/cspPopup?Oform=sub", "cspPopup", "width=600, height=400,  scrollbars=1");
}

function detail(obj_orderNum){
	j$("input[name=orderNum]").val(obj_orderNum);
	j$("#subGoodsForm").attr("action","/madm/service/management/getSubGoodsDetail");
	j$("#subGoodsForm").submit();
}
		...
		<td>
			...
			<table cellpadding="5" cellspacing="0" border="1" width="100%" style="border-collapse:collapse;" bordercolor="#DDDDDD">
				...
				<tr align="left" height="30px">
					...
					<%-- <td bgcolor="#F5F5F5" align="center"><strong>카테고리</strong></td>
					<td>
			    		<select id="searchMenu" name="searchMenu" >
			    			<option value="">선택</option>
			    			<c:forEach var="list" items="${mentalMenuList}" varStatus="status">
			    				<option value="${list.menuCd}"${list.menuCd == param.searchMenu ? ' selected' : ''}>${list.menuNm}</option>
			    			</c:forEach>
			    		</select>
					</td> --%>
				</tr>
				<tr align="left" height="30px">
					...
					<td>
						<input type="text" id="searchCspNm" name="searchCspNm" value="${param.searchCspNm }" readonly />
						<input type="hidden" id="searchCspCd" name="searchCspCd" value="${param.searchCspCd }" />
					...
					<td>
						<table cellpadding="5" align="" id="chRow" cellspacing="0" border="1" width="100%" style="border-collapse:collapse;">
							...
							<tr align="center" height="30px">
								...
								<td >
									${list.orderStatusNm }
									<c:if test="${list.orderType ne '100978'}">
										<c:if test="${list.orderStatus ne '100783'}">
											<br><input type="button" value="주문취소" onclick="javascript:goOrderCancel('${list.orderNum}');" >
										</c:if>
									</c:if>
								</td>
							</tr>
