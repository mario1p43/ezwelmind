...
function pointValidation(){
	...
	var point = $("#point").val();
	if(point!=null && point!=""){
		//2. 차감할 포인트가 개인 잔여한도를 넘지않는지 확인
		var privateRemainPoint = $("#privateRemainPoint").val();
		privateRemainPoint = parseInt(privateRemainPoint.replace(/,/gi,"")) 
		point = parseInt(point.replace(/,/gi,"")) 
		if(point<0){
			if(privateRemainPoint >= Math.abs(point)){
				var clientCd = $('#clientCd').val();
				var userKey = $('#userKey').val();
				var periodSeq = $('#periodSeq').val();
				givePoint(clientCd,userKey,point,periodSeq);
			}else{
				alert('차감할 포인트가 개인 잔여한도를 초과하였습니다.')
				$("#point").val("");
				$("#count").val("");
				$("#count").focus();
			}
		}else{
			alert('차감할 포인트는 0포인트 미만이여야 합니다.')
			$("#count").focus();
			return;;
		}
		
	}else{
		alert('차감할 포인트가 계산되지 않았습니다.비용 조회 후 회기를 입력해주세요.')
	}
...
function givePoint(clientCd,userKey,point,periodSeq){
	$.ajax({
		url: "/madm/operations/givePointToUser?clientCd="+clientCd+"&userKey="+userKey+"&point="+point+"&periodSeq="+periodSeq+"&pointType=U",
		dataType: 'json',
		success: function (response) {
			...
			if(resultMap.resultValue=='success'){
				...
				alert('포인트가 차감가 실패되었습니다. 관리자에게 문의하세요.')
			}
		}
	});
...
<input name="privateRemainPoint" id="privateRemainPoint" type="hidden" value="${privateRemainPoint}" />
<table cellpadding="5" cellspacing="0" border="1px solid silver" style="border-collapse:collapse; width:1000px;" >
	...
	<tr align="left" height="30px">
		...
		<td bgcolor="#F5F5F5" align="center" width="11%"><strong>차감할회기</strong></td>
    	<td align="left"><input type="text" class="count" id="count" name="count"/></td>
    	<td bgcolor="#F5F5F5" align="center"><strong>총 차감 포인트</strong></td>
    	<td align="left">
    		<input type="text" class="point" id="point" name="point" readonly="readonly"/>
    		<button class="givePoint" type="button" disabled="disabled"> 포인트차감하기</button>
    	</td>
	</tr>
</table>
