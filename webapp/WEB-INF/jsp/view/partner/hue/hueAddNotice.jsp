...
j$(document).ready(function(){
	...
	j$('#updateBtn').click(function() {
		j$("#addNotice").attr("action","/partner/hueNotice/update");
		oEditors[0].exec("UPDATE_CONTENTS_FIELD", []);   // 에디터의 내용이 textarea에 적용된다.
	...
	j$('#deleteBtn').click(function() {
		j$("#addNotice").attr("action","/partner/hueNotice/delete");
		j$.alert('삭제하시겠습니까?',function(){
			alert("삭제되었습니다.");
			location.href = "/partner/hueNotice/delete?dataSeq=" + j$('#dataSeq').val();
		 },function(){
		 	return false;
		 }
 	);
	...
	j$("#cancleBtn").click(function(){
		location.href = "/partner/hueNotice/list?bbsCd=notice&serviceType=hue";
		return false;
	});
...
<form id="addNotice" name="addNotice"  action="/partner/hueNotice/add" method="POST" enctype="multipart/form-data">
